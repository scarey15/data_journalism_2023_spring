...
```{r}
baltimore_co <- read_csv("tabula-baltimore_county_carey,_samantha_log_od.csv", col_names = FALSE) %>% 
  clean_names()
...
A1. 07/14/22 has the most with 23 calls in just that day. Since there is so many rows, I do not believe that there are dates with no calls, because there are 366 rows and I am assuming that is including the leap year date.
...
A2. Saturdays seem to be the most common for 911 calls, I think becasue it is a weekend, and usually that is when partying happens the most so I am not surprised.

```{r}
baltimore_co %>% 
  mutate(day_week = 
    wday(dat_format, label = TRUE)
  ) %>% 
  group_by(day_week) %>%  
  summarize(
    total = n()
...
A4. What races are affected the most, there isn't a race column and since this is a diverse area I would like to see the numbers.
I would also like to see the areas that have the most calls, showing different streets with a higher percentage of calls, or a specific area.
