...
**A3** A Anthony; Brienne Adams; Melissa Benitez; Robert Jiles
...
**Q4** Given the results for Q3, what would have made it easier to find the answer? You don't have to know _how_ to accomplish this; I want you to describe in words what would make the answer easier. 

**A4** I think it would be easier to sort by how many seats are avaliable in each of those classes that the instructor teaches, to know the max number of students allowed in each course taught by that instructor.  
...
**A5** I did get an answer, it is interesting to see that the instructors have numerous amounts of seats in each class. 

```{r}
umd_courses %>%
  group_by(instructors, seats) %>%
  summarize(
    count_classes = n(),
