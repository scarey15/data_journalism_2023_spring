...
**A1.** 13,500: Maryland Democratic Party

```{r}
media_expenses <- maryland_expenses

write_csv(media_expenses, "data/mediia_expenses.csv") %>% 
...
**A2.** They are from candidate committees, it seems to be regarding political matters.  
The topo spenders in Facdebooks ad library are nike, amazon, free shipping, invasion and real estate 2 big brands and the rest are categories.
...
**A3.** Any party runners that are noot republican or democrats. The biggest recipients are republicans, Peter Franchot and Monique Anderson-Walker (D)
...
**A4.** This code only shows the committee name and the total amounts from each committee, arranged from greated to least, and only showing 1,994 results.
...
**A5** The committee of Moore Wes for Maryland seems to have to most amount representing the whole state, but the Blair David for Montgomery is right below it, yet they were not in the running for governor.  the stoory we could have here would be about why this person diidn't make it into the final election?  
