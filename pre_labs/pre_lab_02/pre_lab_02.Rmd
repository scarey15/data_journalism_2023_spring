...
**Answer** OR is more restrictive than AND
...
**Answer:** The year is now shown as a four didget number and represents it as a date column instead of a character.
...
**Answer** It will round down the day of the month, so it will be able to be grouped by month. It more represents the entire month instead of different days.
...
**Answer** It is taking Moore's total votes, showing them, and finding the percentage of voters in each region voted for Moore.
...
**Answer** This task arranges the pct_moore percentages from greatest to least
...
**Answer** I think it is only more interesting because you can see the counties that he lost in, and it is more counties lost in, even though it doesn't really count towards his candidacy. 
...
**Answer** str_to_lower: converts to lowercase
...
**Answer** This code will now convert the names of state to two capital letters to represent it, so it is shorter, it also sorts the states so it is easier to access when looking up certain state records.
...
**Answer** Yes, the code tells me that it will sort by state, and then the total in each state arranged by largest to smallest.
...
1. How can you search for dates of each section of class, taught by a certain professor?

2. Can you mutate and organize what courses are taught at certain times in the term, like specific time in the day?

3.  Is there a way to share the location of where each class taught in each department will be at?
