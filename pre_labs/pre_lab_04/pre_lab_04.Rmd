...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Answer** There are different lines displaying the same companies, some with capitals some lowercase, some non English versions, etc. 
```{r}
...
**Answer** Washington County Museum in the cleaned version is in all caps, along with Charles County Arts Alliance Inc. In the Cleaned version is presented in all caps. It is using key_coollision_merge too add two things together
...
**Task** Run the following code to group by grantee_clean and Grantee, then filter for recipients that start with W. Name three changes the refinr function made in the first 30 results. **Answer** Grouping by grantee and grantee_cleaned side by side, the Washington co. comm. action council inc is shortened to Washington Comm Action Council in grantee_clean. Washington College is now lower-cased, besides the first letter in grantee_cleaned. Washingtoon county commissioners is now lower-cased in grantee_cleaned.
...
**Answer** In this version you are seeing the cleaned version and also selecting the Grantee, grantee_clean, and everything.
