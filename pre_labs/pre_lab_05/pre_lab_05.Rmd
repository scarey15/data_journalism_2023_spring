...
**Answer**
The maryland data is shown as a character/column along with the numbers that go along with it.
```{r}
fentanyl_deaths <- read_csv("tabula-Annual_2020_Drug_Intox_Report2.csv") %>% clean_names()
...
**Task** Load the data without headers by running the codeblock below. What problems remain? 
**Answer** The names of the columns, they are x1, x2 and so forth, it is very confusing.
...
**Task** Run the codeblock below. What else do we need to do?
**Answer** We need to sort the data so there aren't as many gaps in each row/column.
