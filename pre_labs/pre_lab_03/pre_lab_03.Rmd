...
**Task** Run the following codeblock to group by the vendor's zip code and show any that are less than five characters in length. How many zip codes do not have five characters? What do you think is happening with this data? 
**Answer**

```{r}
payments %>%
  filter(`Vendor Zip` == '02241')
  group_by(`Vendor Zip`) %>%
...
### Task 7: Count the number of payments in each category by year:

**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer**
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **Answer** The column names are more organized with no spaces in the names. 
...
**Task** Run the following codeblock to use str_to_upper() to make the source, grantee and description columns upper-cased.
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer** 50
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **Answer** 17,740
...
### Task 10: Clean up city field more with case_when()

**Task** Run the following codeblock to use case_when() to fix zip5 issues using both the exact match method and the str_detect() method.
...
cleaned_md_grants_loans <- md_grants_loans %>%
  ...
  mutate(zip5 = case_when(
    zip5 == "Vario" ~ as.character(NA),
    str_detect(zip5, "^UB7") ~ as.character(NA),
    TRUE ~ zip5
  ))
...
### Task 11: Answer our question!

**Task** Write and run code to answer our original question: which zip code has gotten the most amount of money from the Maryland Tourism Board?
